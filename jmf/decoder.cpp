...
JNIEXPORT jint JNICALL Java_com_omnividea_media_parser_video_Parser_avGetAudioBitsPerSample
  (JNIEnv *env , jobject obj, jlong peer)
{
	Decoder *wrapper;    
	...
	wrapper = (Decoder *) jlong2ptr(peer);
	cout << "Number of bits per sample: " << wrapper->getAudioBitsPerSample() << endl;
  return (jint)wrapper->getAudioBitsPerSample();
}
