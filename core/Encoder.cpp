...
ReturnCode Encoder::testOpen() const
{
   ReturnCode error = opened?OkCode:NotInitializedError;
   return error;
}
ReturnCode Encoder::testClose() const
{
   ReturnCode error = (!opened)?OkCode:AlreadyInitializedError;
   return error;
}

int Encoder::getWidth() const
{
   return width;
}
int Encoder::getHeight() const
{
   return height;
}
double Encoder::getBitRate() const
{
   return bitRate;
}
double Encoder::getFrameRate() const
{
   return frameRate;
}
...
ReturnCode Encoder::chooseAudioCodec(const char *name)
{
      ...
      transitionPictureHeight != newHeight)
   {
      ...
      if(isOk(error))
      {
         img_resample_ctx = sws_getContext(
                              ...
                              SWS_BICUBIC/*sws_flags*/, NULL, NULL, NULL);
