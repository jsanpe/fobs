...
FrameIndex Decoder::frameIndexFromTimeStamp(TimeStamp t) const
{
    return (FrameIndex)(t / FOBS_TIMESTAMP_UNITS_D * getFrameRate());
}
TimeStamp Decoder::timeStampFromFrameIndex(FrameIndex f) const
{
    return (TimeStamp)((double)f / (double)getFrameRate() * FOBS_TIMESTAMP_UNITS_D);
}

bool Decoder::compareTimeStamps(TimeStamp t1, TimeStamp t2) const
{
    bool res =  false;
    if(isVideoPresent()) res = ::abs(t2-t1) <= 500.0/getFrameRate();
    else res = ::abs(t2-t1) <= 500.0/getAudioSampleRate();
    //cout << "T1: " << t1 << " == T2: " << t2 << " : " << res << endl;
    return res;
}

TimeStamp Decoder::pts2TimeStamp(int64_t pts, AVRational *pts_timebase) const{
...
int64_t Decoder::timeStamp2pts(TimeStamp ts, AVRational *pts_timebase) const{
	return (int64_t)(ts * (double)pts_timebase->den / pts_timebase->num / FOBS_TIMESTAMP_UNITS_D);
   }

ReturnCode Decoder::testOpen() const
{
   ReturnCode error = opened?OkCode:NotInitializedError;
   return error;
}
ReturnCode Decoder::testClose() const
{
...
ReturnCode Decoder::close()
{
   ...
   if(isOk(error))
   {
      ...
      if(yuvSwsContext == NULL)sws_freeContext(yuvSwsContext);
      if(rgbSwsContext == NULL)sws_freeContext(rgbSwsContext);
      if(rgbaSwsContext == NULL)sws_freeContext(rgbaSwsContext);
      if(rgb2rgbaSwsContext == NULL)sws_freeContext(rgb2rgbaSwsContext);      
...
ReturnCode Decoder::_open()
{
   ...
   */
   ...
   if(isOk(error))
   {
	...
	{
		cout << "Number of channels: " << inputFile->streams[audioStreamIndex]->codec->channels << " - Requesting: 2"<<endl;
   ...
   if(isOk(error) && isVideoPresent())
   {
      ...
      yuvSwsContext = sws_getContext(this->_getWidth(), this->_getHeight(), inputFile->streams[videoStreamIndex]->codec->pix_fmt, //source
                     this->_getWidth(), this->_getHeight(), PIX_FMT_YUV420P, //destination
                     SWS_BICUBIC, NULL, NULL, NULL);
      rgbSwsContext = sws_getContext(this->_getWidth(), this->_getHeight(), inputFile->streams[videoStreamIndex]->codec->pix_fmt, //source
                     this->_getWidth(), this->_getHeight(), PIX_FMT_RGB24, //destination
                     SWS_BICUBIC, NULL, NULL, NULL);
      rgbaSwsContext = sws_getContext(this->_getWidth(), this->_getHeight(), inputFile->streams[videoStreamIndex]->codec->pix_fmt, //source
                     this->_getWidth(), this->_getHeight(), PIX_FMT_BGRA, //destination
                     SWS_BICUBIC, NULL, NULL, NULL);
	  rgb2rgbaSwsContext = sws_getContext(this->_getWidth(), this->_getHeight(), PIX_FMT_RGB24, //source
									   this->_getWidth(), this->_getHeight(), PIX_FMT_BGRA, //destination
									   SWS_BICUBIC, NULL, NULL, NULL);
	   
      if(yuvSwsContext == NULL || rgbSwsContext == NULL || rgbaSwsContext == NULL || rgb2rgbaSwsContext == NULL)
      {
      	error = MemoryError;
      }
	...
	if(isOk(error) && isAudioPresent())
	{
		if(inputFile->streams[audioStreamIndex]->codec->channels > 2 && audioResampleFlag)
		{
			audioResampler = av_audio_resample_init(2, inputFile->streams[audioStreamIndex]->codec->channels, 
    												getAudioSampleRate(), getAudioSampleRate(),
													inputFile->streams[audioStreamIndex]->codec->sample_fmt, inputFile->streams[audioStreamIndex]->codec->sample_fmt,
												    16, 10, 0, 0.8);
...
double Decoder::getFirstFrameTime() const
{
  return firstVideoFramePosition / FOBS_TIMESTAMP_UNITS_D;
}
double Decoder::getFirstAudioSampleTime() const
{
  return firstAudioFramePosition / FOBS_TIMESTAMP_UNITS_D;
}

omnividea::fobs::uint Decoder::getWidth() const
{
   ReturnCode error = testOpen();
   if(isOk(error))
   {
...
omnividea::fobs::uint Decoder::_getWidth() const
{
   return inputFile->streams[videoStreamIndex]->codec->width;
}
omnividea::fobs::uint Decoder::getHeight() const
{
   ReturnCode error = testOpen();
   if(isOk(error))
   {
...
omnividea::fobs::uint Decoder::_getHeight() const
{
   return inputFile->streams[videoStreamIndex]->codec->height;
}
int Decoder::getBitRate() const
{
   ReturnCode error = testOpen();
   if(isOk(error))
   {
...
TimeStamp Decoder::getDurationMilliseconds() const
{
   return duration;
}
double Decoder::getDurationSeconds() const
{
   return duration/FOBS_TIMESTAMP_UNITS_D;
}

byte Decoder::getCrFactor() const
{
   return 2;
}
byte Decoder::getCbFactor() const
{
   return 2;
}
...
byte *Decoder::getLuminance()
{
   ...
   else
   {
      ...
      //ret = img_convert(yuvPicture, pix_fmt, decodedPicture, dec->pix_fmt, dec->width, dec->height);
      ret = sws_scale(yuvSwsContext, decodedPicture->data, decodedPicture->linesize, 0, dec->height, yuvPicture->data, yuvPicture->linesize);
...
ReturnCode Decoder::reallocTransitionPicture(int newWidth, int newHeight)
{
   ...
   if(transitionPictureBufRgb == NULL || 
      ...
      transitionPictureHeight != newHeight)
   {
      ...
      if(isOk(error))
      {
         ...
         if(isOk(error))
         {
            img_resample_ctx = sws_getContext(this->_getWidth(), this->_getHeight(), pix_fmt, //source
                            				  newWidth, newHeight, PIX_FMT_RGB24, //destination
					                          SWS_BICUBIC, NULL, NULL, NULL); //inputFile->streams[videoStreamIndex]->codec->pix_fmt;
...
byte *Decoder::getRGB(int width, int height)
{
   ...
   if(reallocTransitionPicture(width, height) == 0)
   {
      ...
      sws_scale(img_resample_ctx, decodedPicture->data, decodedPicture->linesize, 0, this->_getHeight(), transitionPictureRgb->data, transitionPictureRgb->linesize);
...
byte *Decoder::getRGB()
{
   ...
   else
   {
      ...
      //ret = img_convert(rgbPicture, pix_fmt, decodedPicture, dec->pix_fmt, dec->width, dec->height);
      ret = sws_scale(rgbSwsContext, decodedPicture->data, decodedPicture->linesize, 0, dec->height, rgbPicture->data, rgbPicture->linesize);
...
byte *Decoder::getRGBA()
{
   ...
   int ret;
   AVPicture *tmpPicture;
   enum PixelFormat pix_fmt=PIX_FMT_BGRA;
   AVCodecContext *dec = inputFile->streams[videoStreamIndex]->codec;
   ...
   else
   {
      ...
      //ret = img_convert(rgbaPicture, pix_fmt, decodedPicture, dec->pix_fmt, dec->width, dec->height);
      //ret = sws_scale(rgbaSwsContext, decodedPicture->data, decodedPicture->linesize, 0, dec->height, rgbaPicture->data, rgbaPicture->linesize);
	   this->getRGB();
	   //ret = sws_scale(rgbSwsContext, decodedPicture->data, decodedPicture->linesize, 0, dec->height, rgbPicture->data, rgbPicture->linesize);	   
	   ret = sws_scale(rgb2rgbaSwsContext, rgbPicture->data, rgbPicture->linesize, 0, dec->height, rgbaPicture->data, rgbaPicture->linesize);
...
byte *Decoder::getRGBA(char *buf)
{
   ...
   
   {
      ...
      //ret = img_convert(&tmpPicture, pix_fmt, decodedPicture, dec->pix_fmt, dec->width, dec->height); Deprecated!
      //ret = sws_scale(rgbaSwsContext, decodedPicture->data, decodedPicture->linesize, 0, dec->height, tmpPicture.data, tmpPicture.linesize);
      this->getRGB();
	   ret = sws_scale(rgb2rgbaSwsContext, rgbPicture->data, rgbPicture->linesize, 0, dec->height, tmpPicture.data, tmpPicture.linesize);
      if(ret < 0) 
      {
         return NULL;
      }
...
int Decoder::getAudioSamplesSize() const
{
  return decodedAudioFrameSize;
}

const char* Decoder::getFileName() const
{
   return filename;
}
...
FrameIndex Decoder::getFrameIndex() const
{
...
double Decoder::getFrameTime() const
{
   return (position - firstVideoFramePosition)/FOBS_TIMESTAMP_UNITS_D;
}

double Decoder::getNextFrameTime() const
{
...
ReturnCode Decoder::readNextFrame()
{
   ...
   while(isOk(error) && !frameReady)
   {
     ...
     int ret = av_read_frame(inputFile, &pkt);
     if (ret < 0) 
     {
        //eofReachedFlag = true;
	error = VideoEndError;
...
ReturnCode Decoder::decodeAudioFrame()
{
   ...
   if(isOk(error))
   {
    ...
    ret = avcodec_decode_audio2(inputFile->streams[audioStreamIndex]->codec, (short*) samples, &data_size, ptr, len);
   ...
   if(isOk(error))
   {
    ...
    else
    {
        ...
        if(audioStreamIndex >= 0 && audioResampler && inputFile->streams[audioStreamIndex]->codec->channels > 2)
        {
            //Resampling needed
			cout << "Resampling" << endl;
            int nSamples = data_size / (inputFile->streams[audioStreamIndex]->codec->channels*(getAudioBitsPerSample()/8));
            ...
            memcpy(samples, resampledAudioBuffer, data_size);
        } 
...
TimeStamp Decoder::getAVPosition() const
{
    if(isVideoPresent())
    {
        return position;
...
ReturnCode Decoder::setPosition(TimeStamp milliseconds)
{
	...
	return VideoStreamMissingError;
}

bool Decoder::isVideoPresent() const
{
  return videoStreamIndex >= 0;
}

bool Decoder::isAudioPresent() const
{
...
omnividea::fobs::uint Decoder::getAudioSampleRate() const
{
  uint res = 0;
  if(isAudioPresent())
  {
...
omnividea::fobs::uint Decoder::getAudioBitRate() const
{
  uint res = 0;
  if(isAudioPresent())
  {
...
omnividea::fobs::uint Decoder::getAudioChannelNumber() const
{
  uint res = 0;
  if(isAudioPresent())
  {
	  res = inputFile->streams[audioStreamIndex]->codec->channels;
	  if(audioResampler && inputFile->streams[audioStreamIndex]->codec->channels > 2)
	  {
		  res = 2;
	  }
  }
...
int Decoder::getAudioBitsPerSample() const
{
  int res = 0;
  if(isAudioPresent())
  {
	  res = av_get_bits_per_sample_format(inputFile->streams[audioStreamIndex]->codec->sample_fmt);
  }
	return res;// > 2?2:res;
}
